...
proc emit_fortran {basename} {
    ...
    puts "! Generated by cmdgraph_gen.tcl from ${basename}.cgl -- do not edit"
    puts "!"
    if {[llength $action_procs] > 0} {
        puts "! Action procedures to implement:"
        puts "!   function f(args, ctx) result(rv)"
        puts "!   use cmdgraph; type(dlist_t), intent(in) :: args"
        puts "!   character(len=*), intent(in) :: ctx; type(action_result_t) :: rv"
        foreach p $action_procs { puts "!   $p" }
        puts "!"
    }
    if {[llength $on_enters] > 0} {
        puts "! on_enter procedures to implement:"
        puts "!   subroutine f(ctx); character(len=*), intent(in) :: ctx"
        foreach p $on_enters { puts "!   $p" }
        puts "!"
    }
...
proc emit_cpp {basename} {
    ...
    if {[llength $action_procs] > 0} {
        ...
        foreach p $action_procs {
            puts "//   cmdgraph::ActionResult ${p}(const cmdgraph::ArgList&, const std::string&);"
        }
        puts "//"
    }
    if {[llength $on_enters] > 0} {
...
proc emit_tcl {basename} {
    set all_procs [collect_procs]

    puts "# Generated by cmdgraph_gen.tcl from ${basename}.cgl -- do not edit"
    puts "#"
    if {[llength $all_procs] > 0} {
        puts "# Procedures to implement:"
        puts "#   [join $all_procs {, }]"
        puts "#"
    }
